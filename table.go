...
func (t *table) getGroup(i hashValue) *group {
	return (*group)(unsafe.Add(unsafe.Pointer(&t.groups), uintptr(i)*unsafe.Sizeof(group{})))
}
...
func (t *table) insert(m *Map, hash hashValue, ent entry) {
	if t == nil {
...
func (t *table) split(m *Map) (oldTab, newTab *table) {
	...
	for i := range t.groups {
		...
		for matches != 0 {
			...
			// probing.
			key := m.sb.Get(m.ib.lookup(ent.seq))
			hash := hash(key)
			tab := oldTab
			if hash&mask != 0 {
				tab = newTab
			}
			tab.insert(m, hash, ent)
